...
def batchmark(img_paths: list, output_dir: str, output_list_path: str) -> None:
  ...
  with open(output_list_path, 'w') as fout:
    for path in img_paths:
      print("即将处理:{}".format(path))
      id = re.findall(PATTERN, path)[-1]
      ROI = face_detect(path)
      if len(ROI) == 0:
        continue
      for i,roi in enumerate(ROI):
        avatar_path = os.path.join(output_dir, id + '_' + str(i) + '.jpg')
        fout.write('{} {}\n'.format(id,avatar_path))
        print('{} {}'.format(id, avatar_path))
        try:
          print("差不多SAVE了:{}".format(avatar_path))
          cv2.imwrite(avatar_path, roi[-1])
          print("SAVE OVER {}".format(avatar_path))
        except:
          continue
