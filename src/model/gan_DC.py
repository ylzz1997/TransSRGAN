...
parser.add_argument('--tmp_path', type=str, default='../../resource/DC_training_temp/', help='path of the intermediate files during training')
...
def initital_network_weights(element):
  if hasattr(element, 'weight'):
    element.weight.data.normal_(.0, .1)
...
class SRGAN():
  def __init__(self):
    ...
    self.label_criterion = nn.BCELoss().to(device)
  ...
  def train(self):
    ...
    while self.epoch <= max_epoch:
      ...
      for i, (avatar_tag, avatar_img) in enumerate(self.data_loader):
        ...
        real_loss_sum = real_label_loss * lambda_adv
        ...
        fake_loss_sum = fake_label_loss * lambda_adv
        ...
        loss_g = label_loss_g  * lambda_adv
        loss_g.backward()
