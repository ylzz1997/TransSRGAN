...
parser.add_argument('--tmp_path', type=str, default='../../resource/test2/', help='path of the intermediate files during training')
...
def main():
  ...
  for j in np.arange(-5.0,5.0,1):
    ...
    for i in range(all_num):
      generate(G, '{}'.format(j),torch.cat([g_noise, g_tag], dim=1),img_num,j)
