...
from new_networks.discriminator import Discriminator
...
parser.add_argument('--batch_size', type=int, default=100, help='training batch size for each epoch')
...
parser.add_argument('--tmp_path', type=str, default='../../resource/training_temp_2/', help='path of the intermediate files during training')
...
parser.add_argument('--gradclip', type=int, default=10, help='Gradient Clipping')
...
class SRGAN():
  def __init__(self):
    ...
    if checkpoint == None:
      ...
      self.G.apply(inplace_relu).to(device)
      self.D.apply(inplace_relu).to(device)
      ...
      self.G = Generator()
      self.G = torch.nn.DataParallel(self.G, device_ids=[0,1,2,3]).to(device)
      self.D = Discriminator()
      self.D = torch.nn.DataParallel(self.D, device_ids=[0,1,2,3]).to(device)
  ...
  def train(self):
    iteration = -1
    label = Variable(torch.ones([batch_size,1],dtype=torch.float32)).to(device)
    ...
    while self.epoch <= max_epoch:
      ...
      for i, (avatar_tag, avatar_img) in enumerate(self.data_loader):
        ...
        #grapclip
        nn.utils.clip_grad_norm_(self.D.parameters(),gradclip, norm_type=2)
