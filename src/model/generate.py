...
parser.add_argument('--tmp_path', type=str, default='../../resource/training_temp_old/', help='path of the intermediate files during training')
parser.add_argument('--tmp_path2', type=str, default='../../resource/training_temp_原图/', help='path of the intermediate files during training')
...
def generate(G, file_name, tags,num,path_epoch):
  ...
  g_noise, g_tag = utils.fake_generator(num, 128, device)
...
def main():
  ...
  for j in range(0,401,20):
    ...
    if not os.path.exists(os.path.join(tmp_path,str(j))):
      os.mkdir(os.path.join(tmp_path,str(j)))
    for i in range(all_num):
      generate(G, 'test_{}'.format(i), ['white hair'],img_num,j)
