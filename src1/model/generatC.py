...
parser.add_argument('--tmp_path', type=str, default='../../resource/training_tempAAAA/', help='path of the intermediate files during training')
parser.add_argument('--model_dump_path', type=str, default='../../resource/gan_models2', help='model\'s save path')
parser.add_argument('--img_num', type=int, default=2, help='img number')
...
def main():
  ...
  for i in range(all_num):
    ...
    if not os.path.exists(tmp_path):
       os.mkdir(tmp_path)
    for i in range(all_num):
      generate(G, 'test_{}'.format(i), ['white hair'],img_num,-1)
